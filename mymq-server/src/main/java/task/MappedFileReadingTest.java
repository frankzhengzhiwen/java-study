...
public class MappedFileReadingTest {
    ...
    private void readIndexFileHeader(MappedByteBuffer mappedByteBuffer) throws JsonProcessingException {
        long beginTimestamp = mappedByteBuffer.getLong();
        LOGGER.debug("IndexFile Header BeginTimestamp:[{}]", MAPPER.writeValueAsString(new Date(beginTimestamp)));
        long endTimestamp = mappedByteBuffer.getLong();
        LOGGER.debug("IndexFile Header EndTimestamp:[{}]", MAPPER.writeValueAsString(new Date(endTimestamp)));
    ...
    private void readCommitLog() throws JsonProcessingException {
        int size = mappedByteBuffer.getInt();
        ...
        LOGGER.debug("Message BornTimestamp:[{}]", MAPPER.writeValueAsString(new Date(bornTimestamp)));
        long bornHost = mappedByteBuffer.getLong();
        LOGGER.debug("Message BornHost:[{}]", bornHost);
        long storeTimestamp = mappedByteBuffer.getLong();
        LOGGER.debug("Message BornTimestamp:[{}]", MAPPER.writeValueAsString(new Date(storeTimestamp)));
